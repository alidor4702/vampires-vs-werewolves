# core/alphabeta_agent_v3.py
"""
Alpha-Beta Agent V3 for Vampires vs Werewolves.

CHANGES FROM V2:
- More AGGRESSIVE: Attacks enemy immediately when safe (>=1.5x), even if humans remain
- Priority: Kill enemy > Capture humans (when kill is guaranteed)
- Also moves towards enemy if we have overwhelming advantage
...
class AlphaBetaAgentV3(Agent):
    """Alpha-Beta agent V3 with AGGRESSIVE enemy hunting."""
    ...
    def _score_move_for_group(self, state, r1: int, c1: int, r2: int, c2: int, num: int,
        ...
        V3 CHANGE: Kill enemy has HIGHEST priority when safe!
        """
        ...
        # NEVER: Attack humans we can't capture safely
        if humans_at > 0 and not self._can_capture_humans_safely(num, humans_at):
            return -150000
        
        # NEVER: Attack enemy in risky zone (random battle)
        if enemy_at > 0 and not self._can_kill_enemy_safely(num, enemy_at):
            return -100000
        
        # === PRIORITY SCORING (V3: AGGRESSIVE) ===
        score = 0.0
        
        # ===== V3 CHANGE: Priority 1 is now KILL ENEMY when safe! =====
        if enemy_at > 0 and self._can_kill_enemy_safely(num, enemy_at):
            # VERY HIGH priority - eliminate the enemy!
            score = 25000 + enemy_at * 200
            # Bonus if this eliminates a significant portion of enemy
            if enemy_at >= enemy_total * 0.3:
                score += 10000  # Big bonus for wiping large enemy group
            # Note: Log is now in _search_best_move_for_group to avoid spam
            return score
        ...
        # ===== V3 CHANGE: Priority 4 - HUNT enemy if we can kill them =====
        # Move towards enemy if we have enough units to kill them
        for er, ec, ecount in enemy_pos:
            if self._can_kill_enemy_safely(num, ecount):
                ...
                if new_dist < old_dist:
                    # Getting closer to an enemy we can kill - very good!
                    hunt_score = 10000 + (10 - new_dist) * 150 + ecount * 50
                    if hunt_score > score:
                        score = hunt_score
        ...
        if capturable:
            ...
            if best_score > float('-inf') and 3000 + best_score > score:
                score = 3000 + best_score
        
        # Priority 6: Endgame (no humans) - hunt or flee
        if not human_pos and enemy_pos:
            ...
            if my_total >= enemy_total:
                # Hunt
                if new_dist < old_dist:
                    endgame_score = 2000 + (10 - new_dist) * 50
                    if endgame_score > score:
                        score = endgame_score
            else:
                # Flee
                if new_dist > old_dist:
                    endgame_score = 2000 + new_dist * 50
                    if endgame_score > score:
                        score = endgame_score
    ...
    def _get_fallback_move(self, state, mr: int, mc: int, mcount: int, 
        ...
        """Fast fallback move for a single group. V3: Prioritize killing enemy."""
        best_move = None
        best_score = float('-inf')
        ...
        for dr in [-1, 0, 1]:
            for dc in [-1, 0, 1]:
                ...
                # V3 CHANGE: Kill enemy has highest priority!
                if enemy_at > 0 and self._can_kill_enemy_safely(mcount, enemy_at):
                    score = 15000 + enemy_at
                # Capture humans
                elif dst.humans > 0 and self._can_capture_humans_safely(mcount, dst.humans):
                    score = 10000 + dst.humans
                # Move towards enemy we can kill
                elif enemy_pos:
                    for er, ec, ecount in enemy_pos:
                        if self._can_kill_enemy_safely(mcount, ecount):
                            new_dist = chebyshev_distance(nr, nc, er, ec)
                            hunt_score = 5000 - new_dist * 10
                            if hunt_score > score:
                                score = hunt_score
                # Move towards humans
                if score <= 0 and human_pos:
