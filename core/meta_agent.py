# core/meta_agent.py
"""
Meta Agent v2 - Hybrid Heuristic + Alpha-Beta Search Agent
...
4. Quality splitting - minimum 5 units per split
...
class MetaAgent(Agent):
    ...
    # Configuration
    MAX_GROUPS = 5
    MIN_SPLIT_UNITS = 5  # Never create groups smaller than this
    EARLY_GAME_TURNS = 15
    ENDGAME_HUMAN_THRESHOLD = 0.1  # Enter endgame when humans < 10% of initial
    ...
    def _should_stop_deepening(self) -> bool:
        ...
        return self._time_remaining() < (self.time_limit - self.TIME_LIMIT_SOFT)
    ...
    def _iterative_deepening_search(self, state: GameState, my_pos: List, 
        ...
        # Generate alternatives based on heuristic
        alternatives = [heuristic_moves]
        
        # Add some alternatives for single group
        if len(my_pos) == 1:
            r, c, count = my_pos[0]
            all_moves = self._generate_moves_for_group(state, r, c, count)
            scored = [(self._quick_score_move(state, m, my_pos), m) for m in all_moves]
            scored.sort(reverse=True)
            for _, move in scored[:4]:
                if [move] != heuristic_moves:
        ...
        for depth in range(1, self.MAX_DEPTH + 1):
            ...
            for moves in alternatives[:6]:  # Limit alternatives
                if self._time_critical():
                    break
    ...
    def _try_split(self, state: GameState, my_pos: List, enemy_pos: List, 
        ...
        """Quality-focused splitting. Never create groups smaller than MIN_SPLIT_UNITS."""
        ...
        # Need enough units for quality split
        min_total = 2 * self.MIN_SPLIT_UNITS
        if mcount < min_total:
        ...
        early_game = self.turn_count <= self.EARLY_GAME_TURNS
        
        if early_game:
            min_units = self.MIN_SPLIT_UNITS if sparsity > 0.4 else self.MIN_SPLIT_UNITS + 2
            should_split = sparsity >= 0.3 and mcount >= 2 * min_units
        else:
            min_units = self.MIN_SPLIT_UNITS + 2 if sparsity > 0.5 else self.MIN_SPLIT_UNITS + 4
            should_split = sparsity >= 0.4 and mcount >= 2 * min_units
        
        if not should_split:
            self._log(f"[SPLIT] Skipped: sparsity {sparsity:.2f} too low or insufficient units", 1)
            return None
        ...
        if enemy_pos:
            ...
            if min_enemy_dist <= 2 and mcount < closest_enemy[2] * 2.5:
                self._log(f"[SPLIT] Skipped: enemy too close ({min_enemy_dist})", 1)
                return None
            if min_enemy_dist <= 4 and mcount < closest_enemy[2] * 2:
                self._log(f"[SPLIT] Skipped: enemy nearby ({min_enemy_dist})", 1)
                return None
        ...
        for direction, targets in direction_targets.items():
            ...
            dir_values.append({
                ...
                'closest_h': closest[2],
                'closest_d': closest[3],
                'value': total_h / (closest[3] + 1)
            })
        
        dir_values.sort(key=lambda x: x['value'], reverse=True)
        d1, d2 = dir_values[0], dir_values[1]
        
        # Calculate split ensuring minimum units
        min1 = max(d1['closest_h'], self.MIN_SPLIT_UNITS)
        min2 = max(d2['closest_h'], self.MIN_SPLIT_UNITS)
        ...
        # Proportional split
        total_value = d1['total'] + d2['total']
        if total_value == 0:
            return None
        
        split1 = int(mcount * d1['total'] / total_value)
        split1 = max(min1, min(split1, mcount - min2))
        split2 = mcount - split1
        
        # Ensure both pieces are viable
        if split1 < self.MIN_SPLIT_UNITS or split2 < self.MIN_SPLIT_UNITS:
        ...
        if state.in_bounds(nr1, nc1) and state.in_bounds(nr2, nc2):
            self._log(f"[SPLIT] EXECUTING: {mcount} -> {split1} to ({nr1},{nc1}) + {split2} to ({nr2},{nc2})", 0)
            return [
    ...
    def _score_move(self, state: GameState, r1: int, c1: int, mcount: int, r2: int, c2: int,
        ...
        if urgent_merge and len(my_pos) > 1:
            ...
            if other_groups:
                ...
                old_dist = chebyshev_distance(r1, c1, nearest[0], nearest[1])
                new_dist = chebyshev_distance(r2, c2, nearest[0], nearest[1])
                
                if new_dist < old_dist:
                    # Combined strength after merge
                    combined = mcount + nearest[2]
                    largest_enemy = max(e[2] for e in enemy_pos) if enemy_pos else 0
                    
                    # Extra bonus if merge would make us safe
                    survival_bonus = 1000 if combined >= 0.67 * largest_enemy else 0
                    fight_bonus = 2000 if combined >= 1.5 * largest_enemy else 0
                    
                    return 75000 + (10 - new_dist) * 100 + survival_bonus + fight_bonus, f"URGENT_MERGE(d={new_dist})"
    ...
    def _score_endgame_move(self, state: GameState, r1: int, c1: int, mcount: int, r2: int, c2: int,
        ...
        for er, ec, ecount in enemy_pos:
            ...
            if old_dist <= 2 and self._enemy_can_kill_us(mcount, ecount):
                new_dist = chebyshev_distance(r2, c2, er, ec)
                if new_dist > old_dist:
                    return 55000 + new_dist * 100 + quality_bonus, f"FLEE_ENDGAME(d:{old_dist}->{new_dist})"
        
        # MERGE is very important in endgame
        if len(my_pos) > 1:
            other_groups = [(gr, gc, gc_count) for gr, gc, gc_count in my_pos if (gr, gc) != (r1, c1)]
            if other_groups:
                nearest = min(other_groups, key=lambda g: chebyshev_distance(r1, c1, g[0], g[1]))
                ...
                if new_dist < old_dist:
                    return 50000 + (10 - new_dist) * 50 + quality_bonus, f"MERGE_ENDGAME(d={new_dist})"
    ...
    def _score_attack_mode(self, state: GameState, r1: int, c1: int, mcount: int, r2: int, c2: int,
        ...
        if enemy_at > 0 and self._can_kill_enemy(mcount, enemy_at):
            score = 80000 + enemy_at * 100 + quality_bonus
            enemy_significance = enemy_at / enemy_total if enemy_total > 0 else 0
            if enemy_significance >= 0.5:
                score += 10000
            return score, f"KILL_AM({enemy_at})"
        
        # Priority 2: FLEE if in danger (toward allies!)
        for er, ec, ecount in enemy_pos:
            old_dist = chebyshev_distance(r1, c1, er, ec)
            if old_dist <= 2 and self._enemy_can_kill_us(mcount, ecount):
                new_dist = chebyshev_distance(r2, c2, er, ec)
        ...
        if len(my_pos) > 1:
            ...
            if other_groups:
                ...
                if new_dist < old_dist:
                    main = max(my_pos, key=lambda x: x[2])
                    if (r1, c1) != (main[0], main[1]):
                        return 50000 + (10 - new_dist) * 50 + quality_bonus, f"MERGE_SMALL(d={new_dist})"
                    return 40000 + (10 - new_dist) * 30 + quality_bonus, f"MERGE_MAIN(d={new_dist})"
        
        # Priority 4: HUNT (only if we can win)
        if enemy_pos and my_total >= 1.5 * enemy_total:
            for er, ec, ecount in enemy_pos:
                old_dist = chebyshev_distance(r1, c1, er, ec)
                new_dist = chebyshev_distance(r2, c2, er, ec)
                if new_dist < old_dist:
                    return 20000 + (10 - new_dist) * 20 + quality_bonus, f"HUNT({ecount}E)"
        
        # Priority 5: Move toward center for better position
        center_dir = self._find_center_direction(r1, c1)
        move_dir = (r2 - r1, c2 - c1)
        if center_dir == move_dir:
    ...
    def select_action(self, state: GameState) -> List[Move]:
        ...
        if human_pos and len(my_pos) < self.MAX_GROUPS and not urgent_merge and not endgame:
            self._log(f"\n--- SPLIT EVALUATION ---")
        ...
        for mr, mc, mcount in sorted_groups:
            ...
            best_move = self._get_best_move_for_group(
                state, mr, mc, mcount,
                enemy_pos, human_pos, claimed_targets,
